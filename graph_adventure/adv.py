...
def bfs_dead_end(graph, room):
    q = []
    # Create an empty visited 
    visited = set()
    # Add the direction and room to the queue
    q.append([room])
...
while len(graph) != len(roomGraph):
    ...
    if '?' in graph[current_room].values():
        print('yess', graph[current_room].values())
        # Start Moving
        ...
        # print(f"NEED to walk back {route}")
            for route in routes:
